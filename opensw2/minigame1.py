...
# 게임 배경화면
background = pygame.image.load('background1.png')  # 배경화면 사진 로드
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        # 플레이어 사진 불러오기
        self.image = pygame.image.load('캐릭터.png')
...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        # 적 사진 불러오기
        self.image = pygame.image.load('폭탄.jpg')
        # 이미지 크기의 직사각형 모양 불러오기
        self.rect = self.image.get_rect()
        # rec 크기 축소(충돌판정 이미지에 맞추기 위함)
        self.rect = self.rect.inflate(-20,-20)
        print("적: ", self.rect)
        # 이미지 시작 위치 설정
        self.rect.center = (random.randint(40, 860), 0)
    ...
    def move(self):
        ...
        if (self.rect.bottom > 580):
            SCORE += 1
            self.rect.top = 0
            self.rect.center = (random.randint(30, 810), 0)
...
def mini():
    global SPEED
    bgm = pygame.mixer.Sound('fall_backmusic.mp3')
    bgm.play()
    while True:
        ...
        GameDisplay.blit(scores, (10, 550))
        ...
        if pygame.sprite.spritecollideany(P1, Enemies):
            ...
            # 플레이어
            GameDisplay.blit(background, (0, 0))
            image0 = pygame.image.load('캐릭터.png')
            image0.get_rect()
            GameDisplay.blit(image0, player_pos)

            # 폭탄
            image1 = pygame.image.load('폭탄.jpg')
            image1.get_rect()
            ...
            GameDisplay.blit(final_scores, (250, 180))
            GameDisplay.blit(game_over, (270, 350))
            time.sleep(1)
            pygame.display.update()
            time.sleep(3)
            break
